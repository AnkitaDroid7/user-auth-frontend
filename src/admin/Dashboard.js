...
const Dashboard = () => {
    ...
    const getBookings = () => {
        axios.get('/api/bookings')
            .then((data) => {
                ...
                axios.get(`/api/bookings/${data.data[0]._id}`)
            })
    }
    ...
    const openBooking = async (bkng, e) => {
        ...
        if (!bookingImages[bkng._id]) {
            const data = await axios.get(`/api/bookings/${bkng._id}`)
                                    ...
                                    .map((booking, i) =>
                                        <Card key={i} className="mb-2 shadow-sm">
                                            <Card.Body className='d-flex justify-content-between align-items-center'>
                                                ...
                                                <div>
                                                    <Button size="sm" variant="primary" onClick={() => updateBooking(booking._id, 'SCHEDULED')}>Schedule</Button>
                                                    <span className='px-1'>{' '}</span>
                                                    <Button size="sm" variant="outline-danger" onClick={() => updateBooking(booking._id, 'SPAM')}>Spam</Button>
                                                </div>
                                            </Card.Body>
                                        </Card>
                                    )
                                    ...
                                    .map((booking, i) =>
                                        <Card key={i} className="mb-2 shadow-sm">
                                            <Card.Body className='d-flex justify-content-between align-items-center'>
                                                <div><a href='/' style={{ textDecoration: 'none' }} onClick={(e) => openBooking(booking, e)}>{booking.name}</a></div>
                                                <div>
                                                    <Button size="sm" variant="success" onClick={() => updateBooking(booking._id, 'DONE')}>Done</Button>
                                                </div>
                                            </Card.Body>
                                        </Card>
                                    )
